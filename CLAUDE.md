...
Local-first knowledge-graph workstation. C++ core, Electron/TypeScript shell, local Claude CLI.

Desktop first. Web platform is phase 2 — see "Phase 2 seams" below.

## Non-negotiables

1. **The Claude CLI rides the subscription, never API credits.**
   `app/main/claude/bridge.ts` MUST strip every key in `STRIPPED_ENV_KEYS`
   (`shared/types/claude.ts`) from the child environment before spawning.
...
| `app/main/` | TS | Window lifecycle, IPC, vault file storage, Claude CLI bridge |
